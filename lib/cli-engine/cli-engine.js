...
class CLIEngine {
    ...
    async executeOnFiles(patterns) {
        ...
        for (const { config, filePath, ignored } of fileEnumerator.iterateFiles(patterns)) {
            ...
            asyncWork.push(new Promise(resolve => {

                // TODO: push work to worker pool as callback
                ...
                resolve({ filePath, config, result });

                // TODO: in worker message receive, convert to promise result
            }));
        }

        await Promise.all(asyncWork).then(asyncResults => {

            asyncResults.forEach(lintResult => {
                 ...
                 */
                if (lintResultCache) {
                    lintResultCache.setCachedLintResults(lintResult.filePath, lintResult.config, lintResult.result);
                }
            });
        });
